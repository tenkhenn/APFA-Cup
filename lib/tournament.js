...
export function computeGroupStandings(groupLetter, matches) {
...
export function computeAllStandings(matches) {
  const out = {};
  for (const g of Object.keys(GROUPS)) out[g] = computeGroupStandings(g, matches);
  return out;
}

// Get group winner (only if all 3 matches completed)
export function getGroupWinner(groupLetter, matches) {
  ...
  const standings = computeGroupStandings(groupLetter, matches);
  return standings[0]?.team || null;
}

// Get winner of a knockout match by slot
export function getSlotWinner(slot, matches) {
  ...
  return null; // draws not allowed in knockout but guard anyway
}

// Recompute knockout bracket teams based on completed group/QF/SF matches
export function syncBracket(matches) {
  ...
  for (const p of QF_PAIRINGS) {
    ...
    if (!qf) continue;
    const winA = getGroupWinner(p.groupA, updated);
    const winB = getGroupWinner(p.groupB, updated);
